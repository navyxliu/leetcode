...
public:
    vector<vector<string>> findLadders(string beginWord, string endWord, unordered_set<string> &wordList) {
        ...
        for (int i=0, len=s.size(); i<len; ++i) {
          for (int ch='a'; ch <= 'z'; ++ch) {
            ...
            if (wordList.find(tmp) != wordList.end()) {
              ...
              if (step > next.d) continue; //
              else if (step < next.d) {
                Q.push(tmp);
                next.d = step; 
              }

              next.edges.push_back(s);

              if (tmp == endWord) min = step;
            }
          }
        } 
      }//end of BST 
      ...
      vector<string> wip(min+1);
      wip[min] = endWord;
      backtrace(endWord, beginWord, min+1, 1, wip, ans);
      return ans; 
    }

    void backtrace(const string& s, const string& tgt, int sz, int i, vector<string>& wip, vector<vector<string>>& ans) {
      //cout << "backtrace: " << s << "\n";
      if (s == tgt) {
        ans.push_back(wip);
      } 
        ...
        for (auto& e: node.edges) {
          cout << "idx " << sz - i - 1 << "\n";
          wip[sz-1-i] = e;
          backtrace(e, tgt, sz, i+1, wip, ans);  
        } 
      }
    }
};
